...
library(future) # so we can multi-core
library(future.apply) # so we can multi-core
```
...
*We did not measure the sperm competitive ability of SDSD homozygotes, because _SD-5_ and _SD-72_ are homozygous lethal. _SD-Mad_ is homozygous viable, but a previous study (Wong and Holman, 2019) indicates that the fitness of _SD-Mad_ homozygotes is very close to 0.

```{r Enter estimated parameter values}

# k_c values

estimated_k_SD5 <- 0.942 # 94.4% of progeny inherit SD #0.944

estimated_k_SD72 <- 0.911 # 90.9% of progeny inherit SD #0.909

estimated_k_SDMAD <- 0.867 # 86.8% of progeny inherit SD #0.868
...
estimated_prob_remating_W = 0.3061 # 0.30

estimated_prob_remating_SD5 = 0.753 # 0.75

estimated_prob_remating_SD72 = 0.425 # 0.43

estimated_prob_remating_SDMAD = 0.425 # 0.43
...
- $P1_{normal}$ is the average P1 for the population, where $0 \le P1_{normal} \le 1$. Estimates generally fall between 0 to 0.3 **references here**.
...
While not measured in our study, we assume individuals with the genotype SDSD do not survive to adulthood (`egg_adult_viability` = 0), and that the viability of WSD indivduals is unaffected.
...
if(!file.exists("simulation_results/output2.rds")){
  output2 <- future_lapply(1001:2000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output3.rds")){
  output3 <- future_lapply(2001:3000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output4.rds")){
  output4 <- future_lapply(3001:4000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output5.rds")){
  output5 <- future_lapply(4001:5000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output6.rds")){
  output6 <- future_lapply(5001:6000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output7.rds")){
  output7 <- future_lapply(6001:7000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output8.rds")){
  output8 <- future_lapply(7001:8000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output9.rds")){
  output9 <- future_lapply(8001:9000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output10.rds")){
  output10 <- future_lapply(9001:10000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output11.rds")){
  output11 <- future_lapply(10001:11000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output12.rds")){
  output12 <- future_lapply(11001:12000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output13.rds")){
  output13 <- future_lapply(12001:13000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output14.rds")){
  output14 <- future_lapply(13001:14000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output15.rds")){
  output15 <- future_lapply(14001:15000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output16.rds")){
  output16 <- future_lapply(15001:16000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output17.rds")){
  output17 <- future_lapply(16001:17000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output18.rds")){
  output18 <- future_lapply(17001:18000, run_simulation, parameters = parameters)
...
if(!file.exists("simulation_results/output19.rds")){
  output19 <- future_lapply(18001:19000, run_simulation, parameters = parameters)
...
# Combine the SD freqs with the parameter data and transform data to improve clarity
...
(p1 <- 
    ...
    filter(k == 0.942, `Wildtype P1` == 0.1) %>% 
    ggplot(aes(x = P1_actual, y = P2_actual, fill = 100 * prop_SD)) +
    geom_blank() +
    geom_raster() + 
    stat_contour(aes(z = 100 * prop_SD), colour = "grey10", binwidth = 5) +
    geom_point(data = empirical_estimates_0.1,
               aes(x = P1_actual, y = P2_actual), 
               pch = 21, fill = "#eb7f86", colour = "grey10", size = 2.5) +
    facet_wrap(`Relative risk of sperm competition` ~ `Relative mating success`, 
               scales = "free", nrow = 3, labeller = "label_both", strip.position = c("top")) +
...
(p2 <-
  ...
  rename(`Wildtype P1` = normalP1,
         `Relative risk of sperm competition` = remating_ratio,
         `Relative mating success` = S_precop_WSD_male) %>% 
  filter(k == 0.942, `Wildtype P1` == 0.5) %>% 
  ggplot(aes(x = P1_actual, y = P2_actual, fill = 100 * prop_SD)) +
  geom_blank() +
  geom_raster() + 
  stat_contour(aes(z = 100 * prop_SD), colour = "grey10", binwidth = 5) +
...
**Figure S8**: Predicted equilibrium frequency of the _SD_ allele, calculated from the population genetic model. The plot depicts the interaction between the P1 (x-axis) and P2 (y-axis) costs suffered by _SD/+_ males in their effects on the equilibrium frequency of _SD_ (shown by the colour scale and 5% contour lines). The plot is split into nine panels, with varying levels of _SD/+_ male mating success across the rows and increasing likelihoods of a female remating after first mating to an _SD/+_ male, $p_{SD/+}$, down the columns. The three orange points in each panel represent males carrying the _SD-5_ (leftmost point), and _SD-72_ and _SD-Mad_ (rightmost points) alleles; these points show where males carrying these alleles fall in the figure’s parameter space, as measured in Experiment 2. In the parameter space presented here, $k$ = 0.944 (our highest estimated value), $P1_{normal}$ = 0.5, _SD_ homozygotes are non-viable and _SD_ heterozygotes suffer no fitness costs outside of mating success and sperm competition.
