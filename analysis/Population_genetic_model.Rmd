...
# Run the simulation with just homozygote lethality

$~$

We can find _SD_ allele frequencies for varying levels of segregation distortion if the only fitness cost associated with SD is homozygote lethality. This represents an upper bound for the frequency of _SD_ alleles in our fully parameterised simulation. 
...
For example, when $k$ = 0.944, we find that the _SD_ allele increases from a frequency of 0.1% to 37.8% in 1000 generations, which we use to represent the equilibrium frequency.

[Bruck (1957)](http://dx.doi.org/10.1073/pnas.43.1.152) showed that homozygous lethal segregation distorting alleles (that are associated with no other fitness costs) reach an equilibrium frequency in adults at $q = \frac{1}{2} - \frac{\sqrt{k(1 - k)}}{2k}$, where $k$ is the strength of segregation distortion.

To show the equivalence between Bruck's model and ours, let $k$ = 0.944.

The equilibrium frequency is

```{r, echo=TRUE}
1/2 - sqrt(0.944 * (1- 0.944)) / (2 * 0.944)
```
...
parameters_default %>%
  mutate(prop_SD = round(default_final_SD_props_among_adults, 3)) %>% 
  select(-c(generations, normalP1, initial_freq_SD, parameter_space_ID)) %>% 
