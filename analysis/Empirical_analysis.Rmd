...
new <-
  expand.grid(total_female = 1, SD = unique(drive_test_data$SD), SEX = unique(drive_test_data$SEX)) %>% 
  mutate(Var1 = 1:8)
...
posterior_prediction <- 
  ...
  tidyr::gather(key = id, value = prop_focal_offspring, -posterior_draw) %>%
...
The proportion of female offspring that inherited an _SD_ allele from a heterozygous parent _k_, split by the sex of the parent. _SD_ only drives when present in males. n = 14-15 for all groups.
...
**Figure S2**: The estimated proportion of female offspring that inherited an _SD_ allele from a heterozygous parent, split by the sex of the parent. Black points indicate the estimated mean with associated 66 and 95% uncertainty intervals, while coloured area shows the posterior distribution. The dotted line indicates 50% inheritance; the expectation in the absence of segregation distortion. 

$~$

From these predictions we can calculate viability corrected estimates of _k_ using the formula presented in [Temin (1991)](https://www.ncbi.nlm.nih.gov/pubmed/1906417).
...
**Table S2**: the viability corrected inheritance of each _SD_ variant and the control allele from a _SD/+_ (or _+/+_) male.
...
k_c_SD5 <-
  ...
  mutate(Female_SD_progeny = inv_logit_scaled(b_Intercept + b_SD5),
         Female_bw_progeny = 1 - Female_SD_progeny,
         W_SD5 = Female_bw_progeny / Female_SD_progeny,
         Male_SD_progeny = inv_logit_scaled(b_Intercept + `b_SEXM:SD5`),
         Male_bw_progeny = 1 - Male_SD_progeny) %>% 
...
k_c_SD72 <-
  ...
  mutate(Female_SD_progeny = inv_logit_scaled(b_Intercept + b_SD72),
         Female_bw_progeny = 1 - Female_SD_progeny,
         W_SD72 = Female_bw_progeny / Female_SD_progeny,
         Male_SD_progeny = inv_logit_scaled(b_Intercept + `b_SEXM:SD72`),
         Male_bw_progeny = 1 - Male_SD_progeny) %>% 
...
k_c_SDMAD <-
  ...
  mutate(Female_SD_progeny = inv_logit_scaled(b_Intercept + b_SDMAD),
         Female_bw_progeny = 1 - Female_SD_progeny,
         W_SDMAD = Female_bw_progeny / Female_SD_progeny,
         Male_SD_progeny = inv_logit_scaled(b_Intercept + `b_SEXM:SDMAD`),
         Male_bw_progeny = 1 - Male_SD_progeny) %>% 
...
k_c_W1118 <-
  ...
  mutate(Female_W1118_progeny = inv_logit_scaled(b_Intercept),
         Female_bw_progeny = 1 - Female_W1118_progeny,
         W_W1118 = Female_bw_progeny / Female_W1118_progeny,
         Male_W1118_progeny = inv_logit_scaled(b_Intercept + b_SEXM),
         Male_bw_progeny = 1 - Male_W1118_progeny) %>% 
...
predictions_precop_success <- 
  ...
  as_tibble() %>% 
  group_by(SD) %>% 
  summarise(`Estimated prop. of males mating` = median(prop_SD_success),
            `2.5%` = quantile(prop_SD_success, probs = 0.025),
            `97.5%` = quantile(prop_SD_success, probs = 0.975))
...
The estimated proportion of _SD/+_ or control males that successfully mated with a female when competing with a single _Lbw_ male.
...
The mating success of _SD/+_ males relative to the _w^1118^_ control males. A value of 2 means that _SD/+_ males were twice as successful as _w^1118^_ control males.
...
The mean time taken for _SD/+_ or control males to mate with a _LH~m~_ female.
...
The mean mating latency of males that successfully mated with _LH~m~_ females when competing against _Lbw_ males, relative to the _w^1118^_ control males. A value of 0.5 means that males mated in half the time of _w^1118^_ males.
...
The mean copulation duration when a _SD/+_ or control male mated with a _LH~m~_ female.
...
The mean copulation duration for _SD/+_ males that successfully mated with _LH~m~_ females when competing against _Lbw_ males, relative to the _w^1118^_ control males. A value of 0.5 means that males mated in half the time of _w^1118^_ males.
...
copulation_duration_odds <- 
  ...
  stat_halfeye(aes(fill = parameter), .width = c(0.66, 0.95)) + # width indicates the uncertainty intervals: here we have 66% and 95% intervals
  scale_fill_manual(values = c("SD-Mad" = "#EDAD08", "SD-72" = "#CC503E")) +
  coord_flip() +
  geom_hline(yintercept = 0, linetype = 2) +
  scale_y_continuous("Odds difference from w1118",
...
```{r sperm precedence}
  ...
  mutate(`prop. offspring sired by first mate` = if_else(Mating.Order == "SD/+ male mated first", total_SD / total, total_GFP / total)) %>% 
...
new_posterior_P1 <-
  expand.grid(total_offspring = 1, SD = c("W1118", "MAD", "72", "5"), Block = unique(sperm_comp_data_P1$Block)) %>% 
...
predictions_P1 <- 
  ...
  mutate(posterior_draw = 1:n()) %>%
  gather(key = id, value = prop_focal_offspring, -posterior_draw) %>%
...
The estimated proportion of offspring sired by _SD/+_ or control males, when they mated first.
...
The mean proportion of offspring sired by each _SD_ variant relative to the _w^1118^_ control, when these males mated with _LH~m~_ females first. A value of 0.5 means that the proportion of offspring sired was half that of _w^1118^_ males.
...
mean_P1_plot <-
  ...
  ylab("Estimated prop. of offspring sired when mating first (P1)") +
...
new_posterior_P2 <-
  expand.grid(total_offspring = 1, SD = c("W1118", "MAD", "72", "5"), Block = unique(sperm_comp_data_P2$Block)) %>% 
  mutate(id = paste("V", 1:12, sep = "")) %>% 
  as_tibble()
...
The estimated proportion of offspring sired by _SD/+_ or control males, when they mated second.
...
The mean proportion of offspring sired by each _SD_ variant relative to the _w^1118^_ control, when these males mated with _LH~m~_ females second. A value of 0.5 means that the proportion of offspring sired was half that of _w^1118^_ males.
...
mean_P2_plot <-
  ...
  ylab("Estimated prop. of offspring sired when mating second (P2)") +
  xlab("Variant carried by male") +
  theme_bw() + 
  theme(legend.position = "none",
        panel.grid.minor = element_blank())
...
predictions_remating <- 
  ...
  as_tibble() %>% 
  group_by(SD, Block) %>% 
  summarise(`Estimated prop. of mates remating` = median(prop_females_remating),
            `2.5%` = quantile(prop_females_remating, probs = 0.025),
            `97.5%` = quantile(prop_females_remating, probs = 0.975))
...
The estimated proportion of females that remated that were first mated to a _SD/+_ or control male.
...
The mean proportion of mates remating with _LH~m~^UBI^_ males for _SD/+_ males relative to the _w^1118^_ control males. A value of 2 means that remating was twice as likely when females initially mated with a _SD/+_ male compared with a _w^1118^_ male.
...
mean_remating_plot <-
  ...
  ggplot(aes(SD, prop_females_remating)) + 
  stat_halfeye(aes(fill = SD), .width = c(0.66, 0.95)) + # width indicates the uncertainty intervals: here we have 66% and 95% intervals
  scale_fill_manual(values = c("W1118" = "#38A6A5", "SD-Mad" = "#EDAD08", "SD-72" = "#CC503E", "SD-5" = "#994E95")) +
  coord_flip() +
  ylab("Estimated prop. of females that mated twice") +
...
The estimated time taken for females to remate, when provided with an opportunity, four days after initially mating with a _SD/+_ or control male.
...
The mean remating latency of females that initially mated with _SD/+_ males, relative to the _w^1118^_ control males. A value of 0.5 means that females remated in half the time of females that initially mated with a _w^1118^_ male.
...
The mean copulation duration when a _SD/+_ male mated with a _LH~m~_ female in the first mating role.
...
The mean copulation duration for _SD/+_ males that mated with a _LH~m~_ female in the second mating role, relative to the _w^1118^_ control males. A value of 0.5 means that males mated for half the time of _w^1118^_ males.
...
The mean copulation duration when a _SD/+_ male mated with a _LH~m~_ female in the second mating role.
