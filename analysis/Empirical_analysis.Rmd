...
tibble("Ingredients" = c("Soy flour", "Cornmeal", "Yeast", "Dextrose", "Agar", "Water", "Tegosept", "Acid mix (4ml orthophosphoric acid, 41ml propionic acid, 55mL water to make 100mls)"),
...
## Experiment 1: Testing whether the lines exhibit meiotic drive
...
white.MALE: the number of adult male offspring with white eyes (inheritance of the _SD_ allele is unkown for these offspring when the experimental individual was female)
...
## Experiment 2: Testing the mating success of _SD/+_ males in the presence of a competitor
...
Rearing_vial: the vial the individual male developed in. This variable is included to capture variation explained by the rearing enviornment e.g. small differences in food moisture content or quantity. 
...
Fit the model using mating latency as the response variable and specifiyng a *weibull distribution* (a time-to-event model).
...
Fit the model using copulation duration as the response variable and specifiyng a *weibull distribution* (a time-to-event model).
...
## Experiment 3: Testing sperm competitive success and female remating frequency
...
**Table S9b**: the mean proportion of mates remating with _LH~m~^UBI^_ males for _SD/+_ males relative to the _w^1118^_ control males. A value of 2 means that remating was twice as likely when females intially mated with a _SD/+_ male compared with a _w^1118^_ male.
...
Fit the model - this time using remating latency as the response variable and specifiyng a weibull distribution (a time-to-event model).
...
**Table S10b**: the mean remating latency of females that initially mated with _SD/+_ males, relative to the _w^1118^_ control males. A value of 0.5 means that females remated in half the time of females that intially mated with a _w^1118^_ male.
...
**Figure S5:** The effect that _SD/+_ males have on female remating latency and how this compares to _w^1118^_ control males. Panel **a** shows the estimated remating latency for females exposed to _LH~m~^UBI^_ males over a three-hour period, that had mated with a _SD/+_ or control male four days earlier. Block 2 produced very different results to Blocks 1 and 3, so we exclude it from panel **a** to aid visual clarity (differences between the _SD_ variants and the control do not change). Panel **b** shows effect sizes on the odds scale for the _SD_ variants. Negative values indicate that the mates of _SD/+_ males remated faster than the mates of _w^1118^_ control males. Black points indicate the estimated mean with associated 66 and 95% uncertainty intervals, while coloured area shows the posterior distribution.
...
**Figure 1:** The effect of _SD_ on male mating and fertilisation success. Black points indicate the estimated mean, with associated 66 and 95% uncertainty intervals, while coloured area shows the posterior distribution. Panels a, c, e and g show results on the response scale, while panels b, d, f, and h show the distribution of the log-odds differences between the _SD_ variants and the control allele; 95% uncertainty intervals that do not overlap zero indicate a significant effect. Panel **a** shows the estimated percentage of males that successfully mated with a _LH~m~_ female, when competing against a single _Lbw_ male. Panel **b** shows effect sizes on the log-odds scale for the _SD_ variants; positive values indicate that _SD/+_ males were more likely to mate than the _+/+_ control males. Panel **c** shows the estimated percentage of offspring sired by males carrying _SD_ or the control allele, when these males mated first (P1). Panel **d** shows effect sizes on the log-odds scale for the _SD_ variants; negative values indicate that _SD/+_ males had lower P1 values than the _+/+_ control males. Panel **e** shows the estimated percentage of offspring sired by males carrying _SD_ or the control allele, when these males mated second (P2). Panel **f** shows effect sizes on the log-odds scale for the _SD_ variants; negative values indicate that _SD/+_ males had lower P2 values than the _+/+_ control males. Panel **g** shows the estimated percentage of females remating with _LH~m~^UBI^_ males in a three-hour period, that had mated with a _SD/+_ or control male four days earlier. Data was quite variable between blocks for this response, so we plot model estimates based upon Block 1 to aid visual clarity (differences between the _SD_ variants and the control do not change). Panel **h** shows effect sizes on the log-odds scale for the _SD_ variants; positive values indicate that the mates of _SD/+_ males were more likely to remate than the mates of _+/+_ control males, four days after the first mating.
